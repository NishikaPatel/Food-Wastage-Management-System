...
require_once './mailconfig_agent.php';
if (isset($_POST["sub"])) {
  ...
  $sql = "SELECT COUNT(*) AS count from mail_agent where email = :email_id";
  try {
    $stmt = $DB->prepare($sql);
    ...
    if ($result[0]["count"] > 0) {
      ...
      $sql = "INSERT INTO `mail_agent` (`name`, `pass`, `email`) VALUES " . "( :name, :pass, :email)";
      $stmt = $DB->prepare($sql);
      ...
      if ($result > 0) {
        ...
        $message .= '<p><a href="http://localhost/finalproject/index_agent.php">CLICK TO LOGIN INTO YOUR ACCOUNT</a><p>Welcome!! your account has been successfully created, Thank you.
		</p>';
        $message .= "</body></html>";
  ...
  <div class="row">
    <div class="col-lg-6">
      <div class="well contact-form-container">
        <form class="form-horizontal contactform" action="mailindex_agent.php" method="post" name="f" onsubmit="return validateForm();">
          <fieldset>
