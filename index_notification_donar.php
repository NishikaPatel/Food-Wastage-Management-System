...
<html lang="en">
  ...
  <body background="notificationdonarback.png">
  ...
  $sql_get = mysqli_query($conn,"SELECT * FROM message_donar WHERE status=0");
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav ml-auto">
     <li class="nav-item dropdown">
		...
		<?php
		 $sql_get1 = mysqli_query($conn,"SELECT * FROM message_donar WHERE status=0");
		 if(mysqli_num_rows($sql_get1)>0)
		 {
			while($result = mysqli_fetch_assoc($sql_get1))
			{
			echo '<a class="dropdown-item text-primary" href="read_msg_donar.php?id='.$result['id'].'">'.$result['message'].'</a>';
