<?php 
	...
	if (isset($_POST['reg_user'])) {
		...
		if (count($errors) == 0) {
			...
			$query = "insert into agent_table(username, email, password, age, birth,phone,gender,address)
            VALUES('$username', '$email','$password','$age', '$birth', '$phone','$gender','$address')";
			mysqli_query($db, $query);

			$_SESSION['username'] = $username;
			$_SESSION['success'] = "You are now logged in";
			header('location: mailindex_agent.php');
		}
	...
	if (isset($_POST['login_user'])) {
		...
		if (count($errors) == 0) {
			...
			$query = "SELECT * FROM agent_table WHERE username='$username' AND password='$password'";
